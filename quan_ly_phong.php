...
if (isset($_GET['checkout_id'])) {
    $id = (int)$_GET['checkout_id'];
    $conn->query("UPDATE phong SET trang_thai = 'trong', ma_phieu = NULL WHERE id = $id");
...
if (isset($_GET['checkin_id'])) {
    $id = (int)$_GET['checkin_id'];
    $conn->query("UPDATE phong SET trang_thai = 'da_muon' WHERE id = $id");
    header("Location: quan_ly_phong.php");
    exit;
}

$ds_phong = $conn->query("SET NAMES 'utf8mb4'");
$ds_phong = $conn->query("
    SELECT * FROM phong 
    ORDER BY 
        FIELD(loai_nhom, 'nhỏ', 'vừa', 'lớn'), 
        ten_phong ASC
");

function hien_thi_trang_thai($trang_thai) {
    return match($trang_thai) {
        'da_muon' => 'Đã mượn',
        'trong' => 'Trống',
        default => ucfirst($trang_thai)
    };
}
?>
<!DOCTYPE html>
...
<div class="main">
  <div class="container">
    <h4 class="mb-4">🏠QUẢN LÝ PHÒNG</h4>
    <table class="table table-bordered table-hover bg-white">
      ...
      <tbody>
        <?php $i = 1; while ($row = $ds_phong->fetch_assoc()) { ?>
        <tr>
          <td><?= $i++ ?></td>
          <td><?= $row['ten_phong'] ?></td>
          <td><?= ucfirst($row['loai_nhom']) ?></td>
          <td><?= $row['suc_chua'] ?></td>
          <td><?= hien_thi_trang_thai($row['trang_thai']) ?></td>
          <td>
            <?php if ($row['trang_thai'] === 'da_muon') { ?>
              <a href="quan_ly_phong.php?checkout_id=<?= $row['id'] ?>" class="btn btn-warning btn-sm">🔓 Check out</a>
            <?php } elseif ($row['trang_thai'] === 'trong') { ?>
              <a href="quan_ly_phong.php?checkin_id=<?= $row['id'] ?>" class="btn btn-success btn-sm">🟢 Check in</a>
            <?php } else { ?>
