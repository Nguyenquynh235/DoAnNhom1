<?php
session_start();
$duong_dan = 'lien_he_noi_dung.txt';

// Nếu admin gửi POST thì lưu nội dung
...
<head>
    <meta charset="UTF-8">
    <title>Sửa Liên hệ - Thư viện</title>
...
<div class="main">
    <div class="contact-box">
        <h3 class="text-center text-primary mb-4">📞 Chỉnh sửa thông tin liên hệ</h3>

        <?php if (isset($_SESSION['ban_doc']) && $_SESSION['ban_doc']['vai_tro'] === 'admin'): ?>
            <form method="post">
                <textarea name="noi_dung" class="form-control mb-3" rows="10"><?= htmlspecialchars($noi_dung) ?></textarea>
                <div class="text-center">
                    <button type="submit" class="btn btn-primary">💾 Lưu thông tin</button>
                    <a href="lien_he.php" class="btn btn-secondary">↩️ Quay lại</a>
                </div>
            </form>
        <?php else: ?>
            <div class="alert alert-warning text-center">Chức năng này chỉ dành cho quản trị viên.</div>
        <?php endif; ?>
    </div>
</div>
