...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($_FILES['anh']['tmp_name'], $duong_dan)) {
        $stmt = $conn->prepare("INSERT INTO sach (ten_sach, tac_gia, the_loai, nha_xuat_ban, anh) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $ten_sach, $tac_gia, $the_loai, $nha_xuat_ban, $anh);
        $stmt->execute();
        header("Location: quan_ly_sach.php");
        exit;
    } else {
        $thong_bao = "Lỗi khi upload hình ảnh.";
    }
}
?>
<!DOCTYPE html>
...
<nav class="navbar navbar-light bg-white fixed-top px-4 py-2 d-flex justify-content-between">
    <span class="fw-bold">Thêm Sách Mới</span>
</nav>
<div class="sidebar">
...
<div class="main">
    <div class="container">
        <?php if ($thong_bao) echo "<div class='alert alert-danger'>$thong_bao</div>"; ?>
