...
$ds_phieu = $conn->query("SELECT pm.*, bd.ho_ten FROM phieu_muon pm JOIN ban_doc bd ON pm.ma_ban_doc = bd.ma_ban_doc ORDER BY pm.ma_phieu DESC");
?>
<!DOCTYPE html>
<html lang="vi">
<head>
...
<div class="main">
  <div class="container-fluid">
    <h4 class="mb-4">📋 DANH SÁCH MƯỢN/TRẢ</h4>
    <table class="table table-bordered table-hover bg-white">
      <thead class="table-light">
        <tr>
          <th>Mã Phiếu</th><th>Họ tên</th><th>Ngày mượn</th><th>Ngày trả</th><th>Trạng thái</th>
        </tr>
      </thead>
      <tbody>
        <?php while ($pm = $ds_phieu->fetch_assoc()) { ?>
          <tr>
            <td><?= $pm['ma_phieu'] ?></td>
            <td><?= $pm['ho_ten'] ?></td>
            <td><?= $pm['ngay_muon'] ?></td>
            <td><?= $pm['ngay_tra'] ?></td>
            <td><?= ucfirst($pm['trang_thai']) ?></td>
          </tr>
        <?php } ?>
      </tbody>
    </table>
