...
$ds_thong_ke = $conn->query("SELECT sach.ten_sach, COUNT(*) AS so_lan
                             FROM chi_tiet_muon
                             JOIN sach ON chi_tiet_muon.ma_sach = sach.ma_sach
                             GROUP BY chi_tiet_muon.ma_sach
...
<div class="main">
  <div class="container">
    <h4 class="mb-4">TOP 10 SÁCH ĐƯỢC MƯỢN NHIỀU NHẤT</h4>
    <table class="table table-bordered table-hover bg-white">
      ...
      <tbody>
        <?php $i = 1; while ($s = $ds_thong_ke->fetch_assoc()) { ?>
        <tr>
          <td><?= $i++ ?></td>
          <td><?= $s['ten_sach'] ?></td>
          <td><?= $s['so_lan'] ?></td>
