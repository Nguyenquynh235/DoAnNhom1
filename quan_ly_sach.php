...
if (isset($_GET['xoa'])) {
    $id_xoa = (int)$_GET['xoa'];
    $conn->query("DELETE FROM sach WHERE ma_sach = $id_xoa");
    header("Location: quan_ly_sach.php");
    exit;
}

// Lấy danh sách sách
$ds_sach = $conn->query("SELECT * FROM sach ORDER BY ma_sach DESC");
?>
<!DOCTYPE html>
<html lang="vi">
<head>
...
<div class="main">
    ...
    <a href="them_sach.php" class="btn btn-success">
        ...
        <table class="table table-bordered table-hover bg-white">
            ...
            <tbody>
                <?php while ($row = $ds_sach->fetch_assoc()) { ?>
                    <tr>
                        <td><?= $row['ma_sach'] ?></td>
                        <td><?= $row['ten_sach'] ?></td>
                        <td><?= $row['tac_gia'] ?></td>
                        ...
                        <td><img src="images/<?= $row['anh'] ?>" width="60"></td>
                        <td class="text-center">
                            <div class="d-flex gap-2">
    <a href="sua_sach.php?id=<?= $row['ma_sach'] ?>" class="btn btn-sm btn-primary">
        <i class="fas fa-edit me-1"></i> Sửa
    </a>
    <a href="quan_ly_sach.php?xoa=<?= $row['ma_sach'] ?>" onclick="return confirm('Xác nhận xóa sách này?')" class="btn btn-sm btn-danger">
